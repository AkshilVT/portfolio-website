...
function Navbar({ setActive, active, title }: any) {
  const [showNav, setShowNav] = React.useState(false);
  return (
    <>
      <div className="flex justify-between items-center lg:hidden">
        <Button
          className=" nav-link z-10 min-w-min"
          onClick={() => {
            setShowNav(!showNav);
          }}
        >
      ...
      {showNav && (
        <>
          <div
            className="lg:hidden p-5 fixed inset-10 rounded-lg backdrop-blur bg-white/40 z-50 border-2 border-white/20 shadow-xl duration-100 transition-all"
            id="navbar"
          >
            <GrClose
